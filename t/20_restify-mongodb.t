...
$app->restify->routes($rest, ['test'],
		{collection_method_map => {get  => 'list',
      	post => 'create',put => 'listupdate'}});

my @ids;
# add a record and got id
push @ids, $t->post_ok('/rest/test' => json => {fld => 'value0'})->status_is(200)
     ->json_is('/fld' => 'value0')->tx->res->json->{_id};
# get id
$t->get_ok("/rest/test/" . $ids[0])->status_is(200)->json_is('/_id' => $ids[0]);

# patch a record
$t->patch_ok("/rest/test/" . $ids[0] => json => {fld => 'value00'})
    ->status_is(200)->json_is('/_id' => $ids[0]);
# get patched record
$t->get_ok("/rest/test/" . $ids[0])->status_is(200)->json_is('/fld' => 'value00');
...
my $added = $t->put_ok('/rest/test' => json => \@m_adds)->status_is(200)
    ->json_is('/0/fld' => 'value1')
    ->tx->res->json;
push @ids, $_->{_id} foreach (@$added);

# get 10th-element using filters - q[]
$t->get_ok("/rest/test?q[_id]=" . $ids[10])
    ->status_is(200)->json_is('/0/fld' => 'value10');
$t->get_ok("/rest/test?q[fld]=" . 'value10')
    ->status_is(200)->json_is('/0/fld' => 'value10');

# get value20-value29 - qre[]
$t->get_ok("/rest/test?qre[fld]=" . 'value2[0-9]')
    ->status_is(200)->json_is('/8/fld' => 'value28');

# get first 3 elements of value20-value29 - limit
$t->get_ok("/rest/test?limit=3&qre[fld]=" . 'value2[0-9]')
    ->status_is(200)->json_hasnt('/3')
    ->json_is('/2/fld' => 'value22');
# get 3 page 2, as above but with page - page
$t->get_ok("/rest/test?page=2&limit=3&qre[fld]=" . 'value2[0-9]')
    ->status_is(200)->json_hasnt('/6')
    ->json_is('/2/fld' => 'value25');
...
$t->get_ok("/rest/test?sort[fld]=-1&skip=3&limit=3&qre[fld]=" . 'value2[0-9]')
    ->status_is(200)->json_hasnt('/6')
    ->json_is('/2/fld' => 'value24');

# with_count
$t->get_ok("/rest/test?rc=1&sort[fld]=-1&skip=3&limit=3&qre[fld]=" . 'value2[0-9]')
