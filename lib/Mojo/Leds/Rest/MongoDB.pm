...
sub _patch {
	...
	return $c->_raise_error('Element not found', 404) unless $res->matched_count;
	$rec = {%$rec, %$set};
	return $rec;
}
...
sub _delete {
	my $c = shift;
	my $rec = shift;
	$rec = $c->tableDB->find_one_and_delete(
		{$c->pk => $rec->{_id}}
	);
	return $rec;
}
...
sub list {
	...
	if ($with_count) {
		my $count	= (exists $opt->{limit} || exists $opt->{page}
			|| exists $opt->{skip})
...
sub listupdate {
	...
	foreach my $item (@$json) {
		if (exists $item->{$c->pk}) {
			$c->app->log->debug('Update record ' . Data::Dumper::Dumper($item));
			my $id	= $item->{$c->pk};
			my $rec = $c->_update($item, $id);
			push @recs, $rec;
	} else {
			$c->app->log->debug('Create record ' . Data::Dumper::Dumper($item));
...
sub _qs2q {
	...
	while (my ($k,$v) = each %$flt) {
		...
		given($k) {
			...
			$c->_query_builder(\$qry,$1,$v,sub {return shift})
				when (/^q\[(.*?)\]/);
			# match regexp filter
			$c->_query_builder(\$qry,$1,$v,sub {$a=shift; return qr/$a/i})
				when (/^qre\[(.*?)\]/);
			# advanced sort
	...
	$c->app->log->debug('Query url: ' . Data::Dumper::Dumper($flt)
	 	. "\nSearch: " . Data::Dumper::Dumper($qry)
	 	. "\nOpt: " . Data::Dumper::Dumper($opt));
